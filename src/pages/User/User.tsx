...
import { IUser } from "@/interface/authentication";
...
const User: React.FC = () => {
    ...
    const { deleteUser, loading: processingDelete } = useUser();
    ...
    const getUsers = async (controller?: AbortController) => {
        ...
        try {
            ...
            const response = await authApi.get<PaginationResponse<IUser>>("user", {
                params,
                            ...
                            },
                            {
                                ...
                                render: (record) => (
                                    <Menu shadow="md" position="bottom-end" width={170}>
                                        <Menu.Target>
                                            <ActionIcon variant="filled" aria-label="Settings" disabled={user?.id == record.id}>
